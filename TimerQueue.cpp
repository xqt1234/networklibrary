...
void updateLocInTimerFd(int timeFd, TimeStamp expiration)
{
    ...
    if (expiration.microSinceEpoch() != 0)
    {
...
TimerQueue::TimerQueue(EventLoop *loop)
    : m_loop(loop), m_timerfd(createTimerFd()), m_channel(m_timerfd, loop)
{
...
void mymuduo::TimerQueue::resetTimer(std::vector<Entry> &expiredVec, TimeStamp now)
{
    ...
    if (!m_TimerSet.empty())
    {
        newTime = m_TimerSet.begin()->second->expireTime();
    }
